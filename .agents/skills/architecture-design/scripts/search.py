...
# -*- coding: utf-8 -*-
"""
Architecture Design Pattern Search Engine
Lightweight macro-architecture, system topology, and trade-off search for AI coding agents.
...
DOMAINS = {
    "patterns": "architecture.csv",
    "reasoning": "architecture-reasoning.csv",
    "traps": "architecture-traps.csv",
}
...
def search_domain(domain_name: str, query: str, max_results: int = 3) -> List[Dict[str, Any]]:
    """Search a single architecture domain."""
...
def search_all(query: str, max_results_per_domain: int = 2) -> Dict[str, List[Dict[str, Any]]]:
    """Search across all architecture domains."""
    output = {}
    for domain in ["patterns", "reasoning", "traps"]:
...
def format_record(domain: str, rec: Dict[str, str]) -> str:
    ...
    if domain == "patterns":
        lines.append(f"### [Architecture Pattern] {name} (`{rec_id}`)")
        lines.append(f"- **Paradigm:** {rec.get('paradigm', '')}")
        lines.append(f"- **When to Use:** {rec.get('when_to_use', '')}")
        lines.append(f"- **Avoid When:** {rec.get('avoid_when', '')}")
        lines.append(f"- **Key Components:** {rec.get('key_components', '')}")
        lines.append(f"- **Trade-offs:** {rec.get('trade_offs', '')}")
        lines.append(f"- **Anti-patterns to Avoid:** {rec.get('anti_patterns', '')}")

    elif domain == "traps":
        lines.append(f"### [Architecture Trap] {rec.get('trap_name', name)} (`{rec_id}`)")
        ...
        lines.append(f"### [Architecture Decision] {rec.get('problem_type', name)} (`{rec_id}`)")
...
def main():
    parser = argparse.ArgumentParser(description="Architecture Design Pattern Search")
    parser.add_argument("query", nargs="?", default="", help="Search query or requirements description")
    parser.add_argument("--domain", "-d", choices=list(DOMAIN_ALIASES.keys()), help="Filter by specific domain (patterns, reasoning, traps)")
    ...
    parser.add_argument("--traps", action="store_true", help="Shortcut to display architectural anti-pattern traps")
    ...
    if total_found == 0:
        print(f"## Architecture Design: No exact match found for query: '{args.query}'")
        print("Tip: Broaden your search terms or search without specific domain constraints.")
        return

    print("## Architecture Design — Recommended Patterns\n")
    if args.query:
