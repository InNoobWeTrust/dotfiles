...
Executable QA for web applications: **QA grooming, live browser evidence, scenario-driven audits, optional automation materialization, and audience-adapted reporting**. This skill owns the operational workflow that should stay separate from a pure review.
...
| Intent | Path |
|---|---|
| QA needs help composing an audit-request input file before execution | Grooming path |
...
1. **Grooming / input composition** — interview the QA operator, narrow scope, surface missing assumptions, and draft an auditable audit-request input file before any browser run.
2. **Spot check** — quick, bounded, QA-scoped validation of a named flow with explicit pass/fail/unverified expectations.
3. **Browser audit** — structured pass/fail evidence over one or more scenarios.
4. **Materialization planning** — decide what should become durable automation.
5. **Materialization handoff** — produce the generation contract and outputs needed by a Playwright/Selenium-style implementation.
6. **Stakeholder reporting** — only when audience is non-dev / business / release-owner (or user asks): project machine evidence into Excel, PDF, and/or static HTML under projection gates. Skip entirely for eng-only.
...
- If this skill was not delegated from `reviewer` and the incoming request is still fuzzy, treat that as a routing signal to groom scope first rather than jumping straight into browser mechanics.
