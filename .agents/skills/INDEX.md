...
| `ui-ux` | medium | **Design-driven UI development with UX methodology.** User journey mapping, interaction design, layout planning, visual design, DESIGN.md systems, and frontend implementation with design-code drift prevention. Enforces design-before-code via written UX-SPEC.md contracts. | Non-UI tasks, backend-only work, or tasks where `code-craft` alone suffices (no visual/UX decisions needed) |
| `cdp-browser-automation` | medium | Browser automation, scraping, clicking, filling forms, quick live repros, mechanical page verification, and real page interaction | Static code inspection is enough |
| `web-qa-audit` | medium | Executable web QA/QC: QA grooming to compose audit-request input files, structured browser audits, QA-scoped spot checks with evidence contracts, audience-adapted stakeholder packs (Excel/PDF/HTML), and planning/materializing durable E2E or a11y/regression suites | The task is only heuristic review of an artifact with no executable QA need |
| `data-storytelling` | medium | Turning data into evidence-backed narrative, including explanatory chart selection and Mermaid-based supporting visuals | No dataset or narrative goal exists |
| `illustration-craft` | medium | **Graphic-rich illustrations, infographics, posters, academic figures, and bespoke visual compositions.** When Mermaid or standard charts are insufficient and layout / hierarchy / annotation must carry meaning | Routine chart selection, analytical narrative, UI design systems, or core architecture diagrams already well served by Mermaid / C4 |
| `video-production` | medium | Video production workflow | Non-video tasks |
| `talent-screening` | medium | **Screening CVs, verifying candidate claims, and supporting hiring decisions against a role or JD.** When evaluating job candidates, checking employment/education signals, or performing talent-market research tied to hiring. | General company/lab/market positioning research, private/invasive investigation, or non-hiring strategic intelligence |
| `strategic-osint` | medium | **Public-source intelligence for positioning, targeting, next-action guidance, and long-term direction.** When you need to infer what a person, lab, company, or market likely cares about and adapt messaging or strategy accordingly. | Candidate vetting against a JD, private/invasive investigation, or simple fact lookup with no strategic decision |
...
| `memory` | low–medium | **Two-tier memory for agents (short-term + long-term with dream-cycle consolidation) and the same progressive-disclosure pattern applied to docs and code.** Capture / Recall / Consolidate / Consolidate via Subagent / Evict / Structure. Commit-time consolidation defaults to a report-only subagent pass, with in-agent consolidation only as fallback. Also covers session checkpoint save/restore. | Simple edits with no state to preserve |
| `multi-perspective-deliberation` | low | **Persona debates, pre-mortem challenges, design stress-testing, and strategy deliberation.** Runs concurrent delegated agents/workers if supported or simulated single-context dialogue otherwise. | Straightforward mechanical tasks or simple direct fixes |
| `subagent-dispatch` | low | **Any delegated background execution or external node handoff.** Constructs a delegation prompt that injects scope, structured output, obstacle reporting, and allowed-action constraints directly into the input — compensating for generic worker/agent descriptions. | You have full control over the delegated worker's system prompt and description |
...
| `architecture-design` | medium | **Software architecture work — system design, architecture documentation, pattern selection (Modular Monolith default), modular component design (C4 L1-L3), DDD bounded contexts, ADR writing, architecture audits, API/data/security architecture, and fitness functions.** Responsibility split, data ownership, C4 mermaid diagrams, 33 architecture patterns catalog, scenario workflows, ADR templates. Composes with `db-design` and `code-craft`. | Minor code changes that don't affect architecture |
