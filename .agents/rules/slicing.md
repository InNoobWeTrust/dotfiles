...
1.  **Decompose Vertically**: Group tasks by user story or thin functional flows rather than technical layers. Rank slices by user value and risk reduction, not implementation convenience.
    *   *Lock Cross-Layer Seams First*: Before coding a slice, specify the concrete DTO types and port/service interface signatures connecting the layers in the plan/task. Do not leave layer contracts in prose; locking contract code upfront prevents layers from guessing or drifting.
    *   *Bad (Horizontal)*: "1. Create database schema; 2. Implement API; 3. Create UI."
    *   *Good (Vertical Slice)*: "Slice A: Save a new item (Schema, Repo, API, and barebones UI with locked SaveItemDTO & IItemRepository); Slice B: Render list of items (Repo read, list API, and UI list component with locked ItemSummaryDTO)."
