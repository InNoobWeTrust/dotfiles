# Quality Tooling & Governance

**Read this section when:** selecting or explaining quality tools for developers, AI agents, architects, and management.

| Entry | Summary | Read when |
|---|---|---|
| [Overview](./overview.md) | Key takeaways and problem framing | Starting the section |
| [Mental model](./mental-model.md) | Quality layers and two feedback loops | Choosing tools by role |
| [Choosing tools](./choosing-tools.md) | Fitness criteria and tool groups by layer | Building a baseline stack |
| [Sonar and alternatives](./sonar-and-alternatives.md) | Governance platform vs local loop | Positioning Sonar / Semgrep / CodeQL / etc. |
| [OpenObserve + MegaLinter](./openobserve-megalinter.md) | Self-hosted CI quality telemetry, normalization, and Sonar gaps | Building an OSS-first telemetry plane |
| [Consolidated quality pipeline](./consolidated-quality-pipeline-playbook.md) | MegaLinter, Garden 0.14 DAG, unified CLI, and operational learnings | Building and debugging pipeline DAGs |
| [Maturity and rollout](./maturity-and-rollout.md) | OSS-first path and legacy adoption | Rolling out without backlash |
| [Management metrics](./management-metrics.md) | What leadership should track | Dashboard design |
| [Stack baselines](./stack-baselines.md) | Java, .NET, JS, Python, PHP, Rails, C/C++ | Picking tools per stack |
| [Comparison matrix](./comparison-matrix.md) | Category-level tradeoffs | Fast tool comparison |
| [Agent feedback sensors](./agent-feedback-sensors.md) | Wire quality gates into the coding-agent session | Harness design / Part 2 “leash” |
| [Extended evidence tools](./extended-evidence-tools.md) | Mutation, a11y, fuzz, behavioral hotspots | Filling gaps beyond lint/coverage |

**Related sections:** [project-lifecycle](../project-lifecycle/INDEX.md) · [agentic-qa](../agentic-qa/INDEX.md) · [slides](../slides/INDEX.md) · [Radar Vol 34 research](../research/thoughtworks-radar-vol34/thoughtworks-radar-vol34.md)
