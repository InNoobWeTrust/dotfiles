...
fetch_directory() {
  _owner="$1" _repo="$2" _remote_path="$3" _branch="$4" _local_dir="$5" _depth="${6:-0}"
  ...
  echo "$_listing" | jq -c '.[]' | while IFS= read -r item; do
    ...
    if [ "$_type" = "dir" ]; then
      fetch_directory "$_owner" "$_repo" "$_item_path" "$_branch" "$_local_dir/$_name" $((_depth + 1))
...
sync_item() {
  ...
  if [ "$MODE" = "--apply" ]; then
    ...
    fetch_directory "$_owner" "$_repo" "$_remote_path" "$_branch" "$_local_dir"
...
for SKILL in $(jq -r '.data_sources | keys[]' "$MANIFEST"); do
  jq -c ".data_sources[\"$SKILL\"][]" "$MANIFEST" | while read -r _conf; do
    ...
    _merged_conf=$(echo "$_conf" | jq -c ". + {local_path: \"$_name\"}")
